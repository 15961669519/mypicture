...
void loopNtp() {
  ...
  //格式化得到 时:分:秒
  nt.localTime = timeClient.getFormattedTime();
  ...
  nt.year = ti->tm_year + 1900;
  nt.y = String(nt.year);

  nt.month = ti->tm_mon + 1;
  nt.m = nt.month < 10 ? "0" + String(nt.month) : String(nt.month);

  nt.day = ti->tm_mday;
  nt.d = nt.day < 10 ? "0" + String(nt.day) : String(nt.day);

  nt.hours = ti->tm_hour;
  nt.h = nt.hours < 10 ? "0" + String(nt.hours) : String(nt.hours);

  nt.minutes = ti->tm_min;
  nt.i = nt.minutes < 10 ? "0" + String(nt.minutes) : String(nt.minutes);

  nt.seconds = ti->tm_sec;
  nt.s = nt.seconds < 10 ? "0" + String(nt.seconds) : String(nt.seconds);

  //将得到的年月日写入weather_data 结构体
  nt.localDate = nt.y + "-" + nt.m + "-" + nt.d;
  nt.localHm = nt.h + ":" + nt.i;
