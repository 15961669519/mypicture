...
namespace {
constexpr gpio_num_t kMicPinBclk = GPIO_NUM_5;//面包板
constexpr gpio_num_t kMicPinWs = GPIO_NUM_4;//面包板
constexpr gpio_num_t kMicPinDin = GPIO_NUM_6;//面包板

constexpr gpio_num_t kSpeakerPinBclk = GPIO_NUM_15;//面包板
constexpr gpio_num_t kSpeakerPinWs = GPIO_NUM_16;//面包板
constexpr gpio_num_t kSpeakerPinDout = GPIO_NUM_7;//面包板

constexpr gpio_num_t kI2cPinSda = GPIO_NUM_41;//面包板
constexpr gpio_num_t kI2cPinScl = GPIO_NUM_42;//面包板

constexpr gpio_num_t kTriggerPin = GPIO_NUM_0;//面包板 (复用为配网按键)
constexpr gpio_num_t kLedPin = GPIO_NUM_8;//面包板
...
void InitEspNow();
void SendControlCommand(uint16_t buttons);
void StartDirectionControl(Direction dir, uint16_t button);
void StopDirectionControl();
void CheckDirectionTimeout();
...
void StartDirectionControl(Direction dir, uint16_t button) {
    ...
    SendControlCommand(button);
    printf("开始方向控制: %d\n", dir);
}

void StopDirectionControl() {
    if (activeDirection != NONE) {
        SendControlCommand(0); // 发送停止命令
        printf("停止方向控制: %d\n", activeDirection);
...
void CheckDirectionTimeout() {
    if (activeDirection != NONE && (millis() - directionStartTime >= directionDuration)) {
...
void HandleVoiceCommand(const std::string& command) {
    ...
    // 方向控制（持续3秒后自动停止）
    if (command.find("前进") != std::string::npos || command.find("向前") != std::string::npos) {
        StopDirectionControl();
        StartDirectionControl(FORWARD, BTN_UP);
    } 
    else if (command.find("后退") != std::string::npos || command.find("向后") != std::string::npos) {
        StopDirectionControl();
        StartDirectionControl(BACKWARD, BTN_DOWN);
    } 
    else if (command.find("左转") != std::string::npos || command.find("向左") != std::string::npos) {
        StopDirectionControl();
        StartDirectionControl(LEFT, BTN_LEFT);
    } 
    else if (command.find("右转") != std::string::npos || command.find("向右") != std::string::npos) {
        StopDirectionControl();
        StartDirectionControl(RIGHT, BTN_RIGHT);
    }
    else if (command.find("停止") != std::string::npos || command.find("停下") != std::string::npos) {
        StopDirectionControl();
    }
    
    // 瞬时动作
    else if (command.find("三角") != std::string::npos || command.find("三角形") != std::string::npos) {
        SendControlCommand(BTN_TRIANGLE);
    }
    else if (command.find("圆形") != std::string::npos || command.find("圆圈") != std::string::npos) {
        SendControlCommand(BTN_CIRCLE);
    }
    else if (command.find("叉叉") != std::string::npos || command.find("交叉") != std::string::npos) {
        SendControlCommand(BTN_CROSS);
    }
    else if (command.find("方形") != std::string::npos || command.find("正方形") != std::string::npos) {
        SendControlCommand(BTN_SQUARE);
    }
    else if (command.find("L1") != std::string::npos || command.find("左一") != std::string::npos) {
        SendControlCommand(BTN_L1);
    }
    else if (command.find("L2") != std::string::npos || command.find("左二") != std::string::npos) {
        SendControlCommand(BTN_L2);
    }
    else if (command.find("R1") != std::string::npos || command.find("右一") != std::string::npos) {
        SendControlCommand(BTN_R1);
    }
    else if (command.find("R2") != std::string::npos || command.find("右二") != std::string::npos) {
        SendControlCommand(BTN_R2);
    }
    else if (command.find("L3") != std::string::npos || command.find("左摇杆") != std::string::npos) {
        SendControlCommand(BTN_L3);
    }
    else if (command.find("R3") != std::string::npos || command.find("右摇杆") != std::string::npos) {
        SendControlCommand(BTN_R3);
    }
    else if (command.find("跳舞") != std::string::npos || command.find("舞蹈") != std::string::npos) {
        SendControlCommand(BTN_R1);
    }
    else if (command.find("攻击") != std::string::npos || command.find("打击") != std::string::npos) {
        SendControlCommand(BTN_R2);
    }
}
...
void setup() {
    ...
    // 第三步：初始化IoT实体（关键修复：在引擎启动前注册）
    InitIot();  // 确保LED和扬声器实体提前注册
    ...
    ai_vox_engine.SetTrigger(kTriggerPin);  // 复用引脚作为触发
        ...
        if (auto activation_event = std::get_if<ai_vox::Observer::ActivationEvent>(&event)) {
            ...
            switch (chat_message_event->role) {
                ...
                case ai_vox::ChatRole::kUser: {
                    ...
                    // 处理用户语音命令
                    HandleVoiceCommand(chat_message_event->content);
                    break;
                }
            }
