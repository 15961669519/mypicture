...
const long interval = 1000;  //100表示0.1秒刷新一次屏幕
...
//根据数字长度计算屏幕应该显示的坐标值(1000,16*4/2)
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    u8g2.setCursor(64 - getNumberWidth(i - last_i, 16), y + f);

    u8g2.print(i - last_i); //当前计数减去上一次屏幕刷新时的计数，就是这个时间段内的总转速
    u8g2.sendBuffer();
    last_i = i;
  }
}
